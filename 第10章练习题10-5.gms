$title  例10.6.1，设置价格基准和路易斯宏观闭合

$ontext
路易斯宏观闭合：劳动力价格固定，劳动力数量无限供应，内生决定。
资本要素受限制，由禀赋决定
GDP价格指数认作价格基准
$offtext
...
*瓦尔拉斯法则，这里用加一个虚变量walras的方法
ComEqui(a)..
QA(a)=e=sum(ap,QINT(a,ap))+QH(a)+walras;
...
model checkLewisKup  /all/;
solve checkLewisKup using mcp;

parameter
labormultiplier;
labormultiplier=QLS.l/QLS0-1;

*检验walras是否等于零
display walras.l,QLS0,QLS.l,QKS.l,labormultiplier;
