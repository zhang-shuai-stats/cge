...
$ontext
设置新古典主义宏观闭合
方法3，以劳动价格WL为价格基准，固定WL为1
设置目标函数，增加要优化的目标值，采用NLP将目标函数最小化的方法
$offtext
...
variable
PA(a),PVA(a),PINTA(a),WL,WK,QA(a),QVA(a),QINTA(a),QINT(a,ap),QLD(a),QKD(a),QLS,QKS,YH,QH(a),z;

*定义等式
equation
QAfn(a),QAFOCeq(a),PAeq(a),QVAfn(a),QVAFOC(a),PVAeq(a),QINTfn(a,ap),PINTAeq(ap),
YHeq,QHeq(a),ComEqui(a),Keq,zleq
;
...
*将劳动力市场等式改成劳力市场的过度需求的目标函数
*注意用平方来防止最小化过程进入负数域
zleq..
z=e=sqr(Sum(a,QLD(a))-QLS);
...
*执行优化程序
model cgeNeo  /all/;
solve cgeNeo using nlp minimizing z;
display z.l,PA.l,QA.l;
