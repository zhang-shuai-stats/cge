$title  第15章15.2节 发展中经济，转型经济宏观闭合的CGE模型，研究增值税转型

* 增加了资本供应函数，消费需求函数为LES
...
parameter  rhoAa(a)   /sec1 =   0.2,   sec2soe = 0.5,  sec2pri = 0.5, sec2fdi = 0.5, sec3 = 0.1/
           ...
           Frisch(h)  /hhr       -4,   hhu    -2/;

table      LESelas(c,h)    LES消费需求函数参数
                         hhr     hhu
                 com1    0.5     0.4
                 com2    1.0     1.1
                 com3    1.2     1.4 ;
...
ica(c,a)         中间投入的投入产出系数
shrg(c)          政府支出中对商品c的消费支出份额
shrinv(c)        投资支出额中对商品c的消费支出份额
tih(h)           居民群h的所得税税率
tiEnt            企业所得税税率
tval(a)          对劳动投入的增值税率
tvak(a)          对资本投入的增值税率
...
bgtshr(c,h)      LES函数中居民群h可支配收入中消费预算商品a的份额
bgtshrchk(h)     LES函数中居民群h消费预算商品c的份额之和，检验是否等于1
LESbeta(c,h)     LES居民群h对商品c的边际消费额 �@
LESbetachk(h)    LES居民群h边际消费额参数之和，检验是否等于1
LESsub(c,h)      LES居民群h对商品c消费函数基本生存消费量
tm(c)            进口税率
te(c)            出口税率
PA0(a)           生产活动a的价格
QA0(a)           生产活动a的数量
...
bgtshr(c,h)=SAM(c,h)/EH0(h);
bgtshrchk(h)=sum(c,bgtshr(c,h));
*Below is the LES function block, the LES marginal share coefficient is scaled to satisfy the constraint.
LESbeta(c,h)=LESelas(c,h)*bgtshr(c,h)/(sum(cp,LESelas(cp,h)*bgtshr(cp,h)));
LESbetachk(h)=sum(c,LESbeta(c,h));
LESsub(c,h)=sam(c,h)+(LESbeta(c,h)/PQ0(c))*(EH0(h)/frisch(h));
YENT0=shifentk*WK0*QKSAGG0+transfrEntG0;
QINV0(c)=sam(c,'invsav')/PQ0(c);
EINV0=sum(c,PQ0(c)*QINV0(c));
shrinv(c)=(PQ0(c)*QINV0(c))/EINV0;
...
cpiwt(c)=sum(h,QH0(c,h))/(sum(cp,sum(h,QH0(cp,h))));
CPI0=sum(c,cpiwt(c)*PQ0(c));
kscale=sam('cap','total')/(WK0**kelas);
...
display sax,ica,PQ0,EG0,EG0chk,PGDP0,GDP0,vadded0,GDP0chk,kscale,tvak,bgtshrchk,LESbetachk,shrinv;

variable
PA(a),PVA(a),PINTA(a),WL,WK,QA(a),QVA(a),QINTA(a),QINT(c,a),QLD(a),QKD(a),QX(c),QD(c),QE(c),PX(c),PD(c),PE(c),PQ(c),QQ(c),
PM(c),QM(c),YH(h),EH(h),QH(c,h),QINV(c),QG(c),YENT,ENTSAV,EINV,YG,EG,GSAV,QLSAGG,QKSAGG,FSAV,
GDP,PGDP,CPI,EXR,walras
;

*下面对等式定义
equation

QAfn(a),QAFOCeq(a),PAeq(a),QVAfn(a),QVAFOC(a),PVAeq(a),QINTfn(c,a),PINTAeq(a),QAQXeq(a),PXPAeq(c),CETfn(c),CETFOC(c),PXCET(c),
PEeq(c),QQfn(c),QQfnNoImport(c),QQFOC(c),PQPDCNoImportfn(c),PQeq(c),PMeq(c),YHeq(h),EHeq(h),QHeq(c,h),YENTeq,ENTSAVeq,
QINVeq(c),Ygeq,QGeq,GSAVeq,ComEqui(c),Leq,Keq,Ksupply,FEXeq,ISeq,GDPeq,PGDPeq,CPIeq
;
...
EHeq(h)..
EH(h)=e=mpc(h)*(1-tih(h))*YH(h);

QHeq(c,h)..
PQ(c)*QH(c,h)=e=PQ(c)*LESsub(c,h)+LESbeta(c,h)*(EH(h)-sum(cp,PQ(cp)*LESsub(cp,h)));
