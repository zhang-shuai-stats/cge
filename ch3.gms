...
set ac     总集        /sec1,sec2,sec3,labor,capital,use,total/;
set i(ac)  商品部门集  /sec1,sec2,sec3/;
set x(ac)  要素集      /labor,capital/;
set s(ac)  为了模拟农业价格变化问题建立的子集  /sec2,sec3/;
...
display realq,realf,Q0,a,b,p0,w0,use,lab,cap;
...
parameter
p20(s)       price of sec2 and sec3
p2fx(ss)     price of sector 1
a2(s,ss)     direct input-output coefficients for sec2 and sec3
a22(ss)      direct input-output coefficients for sec1
b2(x,ss)     factor-input-output coefficients for sec2 and sec3
;

p20(s)=1;
p2fx(ss)=1.15;
a2(s,ss)=IO(s,ss)/IO("total",ss);
a22(ss)=IO("sec1",ss)/IO("total",ss);
